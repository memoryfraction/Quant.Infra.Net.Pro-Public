...
Full API documentation available in-app.
...
| Refresh token 7 day expiry | Automated browser re-login on day 6.5, 30-hour retry | Unlimited |
| MFA during re-auth | Dashboard banner + manual recovery | User handles |
| SSL cert expiry | Auto-renewed when < 30 days | 5 years |
| Network disruption | Exponential backoff retry | Auto-recovery |

**Recommendations for 24/7 operation:** Use Windows (for local encryption), stable internet, check Dashboard weekly.
...
| Refresh Token 7 天过期 | 代码操控浏览器重新登录 | 无限制 |
| MFA 挑战 | 仪表盘横幅 + 手动恢复 | 用户处理 |
| SSL 证书到期 | 到期前 30 天自动续期 | 5 年 |
| 网络中断 | 指数退避重试 | 自动恢复 |

**7x24 运行建议：** 使用 Windows、稳定网络、每周检查仪表盘。
