...
| MFA challenge after login | App detects MFA and suspends auto-reauth. User must authorize in browser manually. | Manual intervention required |
| SSL cert expiry | Auto-renewed when < 30 days | 5 years |
| Network disruption | Exponential backoff retry | Auto-recovery |

**Recommendations for 24/7 operation:** Use Windows (for local encryption), stable internet. Note: MFA challenges cannot be automated - monitor and handle manually when notified.
...
| MFA 二次验证 | 应用检测到 MFA 后暂停自动重授权，需用户在浏览器中手动完成授权。 | 需用户手动处理 |
| SSL 证书到期 | 到期前 30 天自动续期 | 5 年 |
| 网络中断 | 指数退避重试 | 自动恢复 |

**7x24 运行建议：** 使用 Windows、稳定网络。注意：MFA 二次验证无法自动化绕过，需关注并及时处理。
...
| MFA challenge appears? | Auto-reauth is suspended. Open browser, re-authorize via Schwab OAuth manually, then restart the app. |
...
| 如何续费？ | 使用购买链接，付款后自动延长。 |
| 浏览器不安全警告？ | 自签名证书的正常现象。 |
| 遇到 MFA 二次验证？ | 自动重授权已暂停。在浏览器中手动完成 Schwab OAuth 授权后重启应用。 |
